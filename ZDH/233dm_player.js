WidgetMetadata = {
  id: "https://cn.233dm.com?rev=20260718c",
  title: "233动漫播放源",
...
async function resolveSource(params, sk) {
  ...
  const sid = SOURCE_IDS[sk];
  const sname = SOURCE_NAMES[sk];
  if (!sid) return [];

  let sr;
  try {
    sr = await Widget.http.get(BASE + "/index.php/ajax/suggest?mid=1&wd=" + encodeURIComponent(seriesName), {
  ...
  for (const item of sd.list) {
    ...
    if (name === seriesName) { best = item; break; }
    if (name.indexOf(seriesName) >= 0) best = item;
  }
  if (!best) best = sd.list[0];
  if (!best || !best.name) return [];

  const hash = await findHash(best.name);
  if (!hash) return [];

  const ep = params.type === "movie" ? 1 : episodeNum;
