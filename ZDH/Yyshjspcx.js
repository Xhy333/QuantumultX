 ...
 * 阴阳师绘卷碎片 - 全自动一体化脚本
 * 功能：
 *  1. 重写触发：自动捕获 GL-Uid/Token/DeviceId 并存至本地
 *  2. 定时任务触发：查询近7天小/中/大绘卷碎片获得数量
 * 配置方式见脚本末尾注释
 *****************************/

// ========== 公用常量 ==========
const STORE_KEY = 'onmyoji_auth';
...
function saveAuth(uid, token, deviceId) {
    const data = JSON.stringify({ uid, token, deviceId });
    $persistentStore.write(data, STORE_KEY);
}
...
function buildHeaders(auth) {
    return {
        ...
        'Origin': 'https://game.16163.com',
        'Referer': 'https://game.16163.com/',
        'User-Agent': USER_AGENT,
        'GL-Uid': auth.uid,
        'GL-Token': auth.token,
        'GL-DeviceId': auth.deviceId,
        'GL-ClientType': '51',
        'GL-Version': '4.17.1',
...
function parseFragments(message) {
    ...
    const large = message.match(/绘卷碎片·大：<span style="color:#ff0000">(\d+)<\/span>/);
    return {
        small: small ? parseInt(small[1]) : 0,
        medium: medium ? parseInt(medium[1]) : 0,
        large: large ? parseInt(large[1]) : 0
    };
}
...
function captureAuth() {
    ...
    const uid = headers['GL-Uid'];
    const token = headers['GL-Token'];
    const deviceId = headers['GL-DeviceId'];

    if (uid && token && deviceId) {
        saveAuth(uid, token, deviceId);
        console.log('✅ 绘卷认证信息已更新: ' + JSON.stringify({ uid, token, deviceId }));
    } else {
        console.log('⚠️ 未捕获完整认证头');
    }
    $done({});
}
...
async function queryAll(days = 7) {
    const auth = getAuth();
    if (!auth) throw new Error('未找到认证信息，请先打开游戏进入绘卷页面一次');
    ...
    for (const dateStr of dayList) {
        ...
        if (data.code !== 200) throw new Error(`初始化会话失败 (${dateStr}): ${resp.body}`);
        ...
        if (status !== 'waiting') throw new Error(`未能进入日期选择 (${dateStr})`);
